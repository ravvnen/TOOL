...
public sealed class SeedHandler : ISeedHandler
{
    ...
    public async Task<PublishResult> HandleAsync(SeedDto dto, string rawJson, string? deliveryId, CancellationToken ct = default)
    {
        dto.Validate();
        ...
        _log.LogInformation("[Seed] Published {Subject} (stream={Stream}, seq={Seq})", subject, ack.Stream, ack.Seq);
    ...
    private static (string org, string repo) SplitOrgRepoOrBadRequest(string value)
    {
        ...
        if (parts.Length != 2) throw new BadHttpRequestException("source.repo must be 'org/repo'");
    ...
    private static string? ExtractOrgRepoFromUrl(string? repoUrl)
    {
        ...
        var url = repoUrl.Contains("://", StringComparison.Ordinal) ? repoUrl : "https://" + repoUrl;
        ...
        var org = segs[0];
        var repo = segs[1].EndsWith(".git", StringComparison.OrdinalIgnoreCase) ? segs[1][..^4] : segs[1];
        return $"{org}/{repo}";
    }
