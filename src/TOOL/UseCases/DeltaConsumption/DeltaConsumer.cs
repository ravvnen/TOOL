...
public sealed class DeltaConsumer : BackgroundService
{
    ...
    public DeltaConsumer(NatsJSContext js, ILogger<DeltaConsumer> log)
    {
        _js = js;
        _log = log;
        _dbPath  = Environment.GetEnvironmentVariable("DELTA_DB_PATH") ?? "deltas.db";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await foreach (var msg in consumer.ConsumeAsync<byte[]>().WithCancellation(stoppingToken))
        {
            try
            {
                ...
                if (!TryGetString(root, "ns", out var ns) ||
                ...
                var labels = root.TryGetProperty("labels", out var labelsEl) && labelsEl.ValueKind == JsonValueKind.Array
                    ? JsonSerializer.Serialize(labelsEl.EnumerateArray().Select(x => x.GetString()).Where(x => !string.IsNullOrWhiteSpace(x)))
                    : "[]";
                var newVersion = root.GetProperty("new_version").GetInt32();
                var baseVersion = root.TryGetProperty("base_version", out var bv) && bv.ValueKind == JsonValueKind.Number ? bv.GetInt32() : 0;
                ...
                var alreadySeen = await db.ExecuteScalarAsync<long>(
                    ...
                    new { Ns = ns, Eid = inputEventId }) > 0;

                if (alreadySeen)
                {
                    _log.LogDebug("Seen delta (ACK fast). ns={Ns} item={Item} id={Id}", ns, itemId, inputEventId);
                    await msg.AckAsync();
                    continue;
                }
                ...
                if (type == "im.upsert.v1")
                {
                    ...
                    await db.ExecuteAsync(@"
                        ...
                        new { Ns = ns, ItemId = itemId, Version = newVersion,
                              Repo = repo, Ref = srcRef, Path = path, BlobSha = blobSha }, tx);
                }
                ...
                await db.ExecuteAsync(
                    ...
                    new { Ns = ns, Eid = inputEventId }, tx);

                tx.Commit();

                _log.LogInformation("Applied {Type} ns={Ns} item={Item} v{V}", type, ns, itemId, newVersion);
                await msg.AckAsync();
            }
